...
package com.loffler.scanServ.cdcgesture.detection;
...
//            canvas.drawRoundRect(trackedPos, cornerSize, cornerSize, boxPaint);
      ...
      canvas.drawPath(createCornersPath((int)trackedPos.left, (int)trackedPos.top, (int)trackedPos.right, (int)trackedPos.bottom, (int) cornerSize), boxPaint);
...
//          canvas, trackedPos.left + cornerSize, trackedPos.top, labelString + "%", boxPaint);
  ...
  private void processResults(final List<Detector.Recognition> results) {
    ...
    for (final Pair<Float, Detector.Recognition> potential : rectsToTrack) {
      ...
      if (trackedRecognition.detectionConfidence >= 0.8f) {
        trackedRecognition.color = COLORS[2];
      } else if (trackedRecognition.detectionConfidence >= 0.75f) {
        trackedRecognition.color = COLORS[3];
      } else if (trackedRecognition.detectionConfidence >= 0.70f) {
        trackedRecognition.color = COLORS[7];
      } else if (trackedRecognition.detectionConfidence >= 0.65f) {
        trackedRecognition.color = COLORS[1];
      } else {
        trackedRecognition.color = COLORS[8];
